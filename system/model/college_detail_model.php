...
class College_detail_Model extends Crexo_Model {
	...
	public function page_title($slug) {
		$query= $this->db->select('colleges',array( 'slug=' => $slug ));
	...
	public function colleges(){
		$rows = $this->db->select('colleges',array('status=' => 1));
		foreach($rows as $row){
			$col[] = $row;
		}
		return $col;
	...
	public static function featured_image($slug,$width='',$height='',$class=''){
		...
		if($slug!=404){
			...
			if(isset($page_featured_image) && $page_featured_image!=''){
				$featured_image = '<img class="'.$class.'" src="'.DATA_VIEW.'/colleges/'.$page_featured_image.'" width="'.$width.'" height="'.$height.'" />';
			} else {
