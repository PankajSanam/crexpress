...
class Product_Controller extends Crexo_Controller {
	...
	public function back_index(){
		...
		$this->data['products_data']=	$this->model[$this->page]->products();
		$this->data['product_categories'] =	$this->model['product']->categories();

		$this->data['breadcrumb'] = '
			<li><a href="product.html">Product Management</a><i class="icon-angle-right"></i></li>
			<li><a href="product.html?product=add">New Product</a></li>
		';
		...
		if(isset($_GET['product']) && $_GET['product'] == 'edit' || isset($_GET['category']) && $_GET['category'] == 'edit' ){
			...
			$this->data['edit_data'] 	= 	array( array() );
		}
		
		if(isset($_GET['feature']) && isset($_GET['action']) && $_GET['action'] == 'edit' ){
			$this->data['edit_feature_data']	= 	$this->model[$this->page]->edit_feature_data($_GET['id']);
		} else {
			$this->data['edit_feature_data'] 	= 	array( array() );
		}
	...
	public function front_index(){
		...
		$this->data['pdf'] = $this->model[$this->page_template]->pdf_name($this->slug);
		$this->data['page_content'] = $this->model[$this->page_template]->page_content($this->slug);
		$this->data['features'] = $this->model[$this->page_template]->features($this->slug);
		$this->data['product_image'] = $this->model[$this->page_template]->product_image($this->slug);
