...
class Pages_Controller extends Crexo_Controller {
	...
	{
		$this->data['meta_title'] 	=	$this->model[$this->slug]->meta_title('Pages - GIT BOX');
		$this->data['pages']		=	$this->model[$this->slug]->pages();
		$this->data['page_templates'] = $this->model[$this->slug]->page_templates();
		$this->data['breadcrumb'] = '
			<li><a href="pages.html">Pages Management</a><i class="icon-angle-right"></i></li>
			<li><a href="pages.html?action=add">Add Pages</a></li>';

		if( get('edit') ){
			$this->data['edit_page']	= 	$this->model[$this->slug]->edit_page(get('edit'));
		} else {
			$this->data['edit_page'] 	=	$this->model[$this->slug]->edit_page(1);
		}
