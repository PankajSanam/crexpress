...
class Ads_Controller extends Crexo_Controller 
{
	public function __construct($slug)
	{
		...
		$scripts = array(
			...
			//Touch enable for jquery UI
				'plugins/touch-punch/jquery.touch-punch.min.js.js',
			//vmap
	...
	public function back_index()
	{
		...
		$this->data['records'] = $model->records();
		$this->data['getRecords'] = array( array() );		

		$this->data['meta_title'] = meta_title('Ads Management - CrexPress');
		$this->data['body_class'] = '';

		$this->data['breadcrumb'] = '
			<li><a href="'.back_url('ads/').'">Ads Management</a><i class="icon-angle-right"></i></li>
			<li><a href="'.back_url('ads/add/').'">Add New</a></li>
		';

		$this->data['page_title'] = 'Ads Management';

		$this->data['activePages'] = $model->activePages();

		if( get('edit') )
		{
			$this->data['getRecords'] = $model->getRecord( get('edit') );
			$this->data['meta_title'] = meta_title('Manage Ads- CrexPress');
			...
			{
				$model->removeImage();
				redirect( back_url('ads/edit/'.get('edit') ) );
			}
		}

		if(isset($_POST['save']))
		{
			if ( get('edit') ) $model->updateRecord();
			if ( get('add') ) $model->addRecord();

			redirect( back_url('ads/') );
		}
